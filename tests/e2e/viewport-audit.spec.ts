 ...
 * 인증 없는 상태에서 접근 가능한 / 와 /onboarding, /login 을 기준으로
...
test.describe('뷰포트 감사: 360 / 375 / 961 / 1440', () => {
  for (const vp of VIEWPORTS) {
    test(`${vp.name} (${vp.width}x${vp.height}): /onboarding Hero 가 렌더되고 CTA 는 40px 이상`, async ({
      page,
    }) => {
      await page.setViewportSize({ width: vp.width, height: vp.height });
      await page.goto('/onboarding');
      const hero = page.locator('[data-slot="onboarding-hero"]');
      await expect(hero).toBeVisible();
      // 터치 타깃 최소 높이 검증 — lg primary CTA 는 h-12(48px)
