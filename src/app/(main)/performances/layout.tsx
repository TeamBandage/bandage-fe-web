...
/**
 * /performances 영역 레이아웃.
 * - 기본: 좌측 ListPane(360px) + 우측 children (master-detail).
 * - /performances/new: 풀페이지 마법사 — ListPane 비노출, children 만 단일 패널로.
 */
export default function PerformancesLayout({ children }: { children: ReactNode }) {
  const pathname = usePathname() ?? '';
  const fullPage =
    pathname === ROUTES.PERFORMANCE_NEW || pathname.startsWith(`${ROUTES.PERFORMANCE_NEW}/`);
